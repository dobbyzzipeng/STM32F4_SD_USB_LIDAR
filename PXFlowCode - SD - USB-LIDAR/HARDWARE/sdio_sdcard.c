...
SD_Error SD_Init(void)
{
    ...
    GPIO_Init(GPIOD, &GPIO_InitStructure);//PB3复用功能输出
	
	 //引脚复用映射设置
	GPIO_PinAFConfig(GPIOC,GPIO_PinSource8,GPIO_AF_SDIO); //PC8->SDIO-D0
    GPIO_PinAFConfig(GPIOC,GPIO_PinSource9,GPIO_AF_SDIO);//PC9->SDIO-D1
    GPIO_PinAFConfig(GPIOC,GPIO_PinSource10,GPIO_AF_SDIO);//PC10->SDIO-D2
    GPIO_PinAFConfig(GPIOC,GPIO_PinSource11,GPIO_AF_SDIO);//PC11->SDIO-D3
    GPIO_PinAFConfig(GPIOC,GPIO_PinSource12,GPIO_AF_SDIO);//PC12->SDIO-SCK
    GPIO_PinAFConfig(GPIOD,GPIO_PinSource2,GPIO_AF_SDIO);//PD2->SDIO-CMD
...
{
  	u1_printf("Card ManufacturerID:%d\r\n",SDCardInfo.SD_cid.ManufacturerID);	//制造商ID
 	u1_printf("Card RCA:%d\r\n",SDCardInfo.RCA);								//卡相对地址
 	u1_printf("Card BlockSize:%d\r\n",SDCardInfo.CardBlockSize);			//显示块大小
	u1_printf("Card Capacity:%d MB\r\n",(u32)(SDCardInfo.CardCapacity>>20));	//显示容量
	switch(SDCardInfo.CardType)
	{
		case SDIO_STD_CAPACITY_SD_CARD_V1_1:u1_printf("Card Type:SDSC V1.1\r\n");break;
		case SDIO_STD_CAPACITY_SD_CARD_V2_0:u1_printf("Card Type:SDSC V2.0\r\n");break;
		case SDIO_HIGH_CAPACITY_SD_CARD:u1_printf("Card Type:SDHC V2.0\r\n");break;
		case SDIO_MULTIMEDIA_CARD:u1_printf("Card Type:MMC Card\r\n");break;
	}	
}
